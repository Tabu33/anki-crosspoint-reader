...
#include "activities/settings/AnkiSyncSettingsActivity.h"
#include "components/UITheme.h"
#include "fontIds.h"

int HomeActivity::getMenuItemCount() const {
  int count = 5;  // File Browser, Recents, File transfer, Settings, Anki
...
void HomeActivity::render(RenderLock&&) {
  ...
  std::vector<const char*> menuItems = {tr(STR_BROWSE_FILES), tr(STR_MENU_RECENT_BOOKS), tr(STR_FILE_TRANSFER),
                                        tr(STR_SETTINGS_TITLE), tr(STR_ANKI)};
  std::vector<UIIcon> menuIcons = {Folder, Recent, Transfer, Settings, Bookmark};
...
void HomeActivity::onAnkiOpen() {
  startActivityForResult(std::make_unique<AnkiSyncSettingsActivity>(renderer, mappedInput),
